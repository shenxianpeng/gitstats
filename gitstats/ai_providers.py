        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    raise AIProviderError(f"Failed after {self.max_retries} attempts: {str(e)}")
...
class OpenAIProvider(AIProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        try:
            ...
            raise AIProviderError("openai package not installed. Install with: pip install openai")
        
        # Get API key from config or environment
        self.api_key = config.get('api_key') or os.environ.get('OPENAI_API_KEY')
        if not self.api_key:
            raise AIProviderError("OpenAI API key not found. Set OPENAI_API_KEY environment variable or configure it.")
    ...
    def generate_summary(self, data: Dict[str, Any], prompt: str) -> str:
        ...
        def _call_api():
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert software development analyst who provides insightful analysis of git repository statistics."},
...
class ClaudeProvider(AIProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        try:
            ...
            raise AIProviderError("anthropic package not installed. Install with: pip install anthropic")
        
        self.api_key = config.get('api_key') or os.environ.get('ANTHROPIC_API_KEY')
        if not self.api_key:
            raise AIProviderError("Claude API key not found. Set ANTHROPIC_API_KEY environment variable or configure it.")
...
class GeminiProvider(AIProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        try:
            ...
            raise AIProviderError("google-generativeai package not installed. Install with: pip install google-generativeai")
        
        self.api_key = config.get('api_key') or os.environ.get('GOOGLE_API_KEY')
        if not self.api_key:
            raise AIProviderError("Gemini API key not found. Set GOOGLE_API_KEY environment variable or configure it.")
...
class OllamaProvider(AIProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        try:
            ...
            raise AIProviderError("requests package not installed. Install with: pip install requests")
...
class CopilotProvider(AIProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not self.api_key:
            raise AIProviderError("GitHub token not found. Set GITHUB_TOKEN environment variable or configure it.")
...
class AIProviderFactory:
    ...
    def create(cls, provider_name: str, config: Dict[str, Any]) -> AIProvider:
        ...
        if provider_name not in cls._providers:
            ...
            raise AIProviderError(f"Unknown AI provider: {provider_name}. Supported providers: {supported}")
