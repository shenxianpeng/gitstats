...
class AISummarizer:
    ...
    def prepare_index_data(self, data: Dict[str, Any]) -> str:
        ...
        top_authors = sorted(human_authors.items(), key=lambda x: x[1].get('commits', 0), reverse=True)[:5]
        top_authors_str = '\n'.join([f"  - {name}: {info.get('commits', 0)} commits" for name, info in top_authors])
...
"""
    ...
    def prepare_activity_data(self, data: Dict[str, Any]) -> str:
        ...
        years_str = '\n'.join([f"  - {year}: {count} commits" for year, count in sorted(commits_by_year.items())])
...
"""
    ...
    def prepare_authors_data(self, data: Dict[str, Any]) -> str:
        ...
        authors_by_commits = sorted(human_authors.items(), key=lambda x: x[1].get('commits', 0), reverse=True)
        ...
        if authors_by_commits:
            top_author_percentage = (authors_by_commits[0][1].get('commits', 0) / total_commits * 100) if total_commits > 0 else 0
...
"""
    ...
    def generate_summary(self, page_type: str, data: Dict[str, Any], force_refresh: bool = False) -> Dict[str, Any]:
        """
...
Generate your analysis:"""
    ...
    def generate_all_summaries(self, data: Dict[str, Any], force_refresh: bool = False) -> Dict[str, Dict[str, Any]]:
        """
