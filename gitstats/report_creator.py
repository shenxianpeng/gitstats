...
from gitstats import load_config, WEEKDAYS
...
class HTMLReportCreator(ReportCreator):
    ...
    def create_graph_hour_of_day(self, path):
        # hour of day
        with open(path + "/hour_of_day.dat") as f:
        ...
        plt.savefig(path + "/hour_of_day.png")
        plt.close()

    def create_graph_day_of_week(self, path):
        # day of week
        with open(path + "/day_of_week.dat") as f:
            data = [line.split() for line in f.readlines()]
        ...
        plt.ylim(0, max(y) * 1.1)
        plt.savefig(path + "/day_of_week.png")
        plt.close()

    def create_graph_domains(self, path):
        # Domains
        with open(path + "/domains.dat") as f:
            data = [line.split() for line in f.readlines()]
        ...
        plt.xticks(rotation=45)
        plt.ylim(0, max(y) * 1.1)
        plt.savefig(path + "/domains.png")
        plt.close()

    def create_graph_month_of_year(self, path):
        # Month of Year
        with open(path + "/month_of_year.dat") as f:
            data = [line.split() for line in f.readlines()]
            x = [int(row[0]) for row in data]
            y = [int(row[1]) for row in data]
        ...
        plt.xlim(0.5, 12.5)
        plt.ylim(0, max(y) * 1.1)
        plt.savefig(path + "/month_of_year.png")
        plt.close()

    def create_graph_commits_by_year_month(self, path):
        # commits_by_year_month
        with open(path + "/commits_by_year_month.dat") as f:
            data = [line.split() for line in f.readlines()]
            x = [row[0] for row in data]
            y = [int(row[1]) for row in data]
        ...
        plt.xticks(rotation=45)
        plt.ylim(0, max(y) * 1.1)
        plt.savefig(path + "/commits_by_year_month.png")
        plt.close()

    def create_graph_commits_by_year(self, path):
        # commits_by_year
        with open(path + "/commits_by_year.dat") as f:
            data = [line.split() for line in f.readlines()]
            x = [int(row[0]) for row in data]
            y = [int(row[1]) for row in data]
        ...
        plt.ylim(0, max(y) * 1.1)
        plt.savefig(path + "/commits_by_year.png")
        plt.close()

    def create_graph_files_by_date(self, path):
        # Files by date
        with open(path + "/files_by_date.dat") as f:
            data = [line.split() for line in f.readlines()]
            x = [row[0] for row in data]
            y = [int(row[1]) for row in data]
        ...
        plt.xticks(rotation=45)
        plt.ylim(0, max(y) * 1.1)
        plt.savefig(path + "/files_by_date.png")
        plt.close()

    def create_graph_lines_of_code(self, path):
        # Lines of Code
        with open(path + "/lines_of_code.dat") as f:
            data = [line.split() for line in f.readlines()]
            x = [datetime.datetime.fromtimestamp(int(row[0])) for row in data]
            y = [int(row[1]) for row in data]
    ...
    def create_graph_lines_of_code_by_author(self, path):
        ...
        with open(path + "/lines_of_code_by_author.dat") as f:
            ...
            plt.legend()
            plt.ylim(bottom=0)
            plt.savefig(path + "/lines_of_code_by_author.png")
            plt.close()

    def create_graph_commits_by_author(self, path):
        # Commits per author
        with open(path + "/commits_by_author.dat") as f:
            data = [list(map(int, line.split())) for line in f.readlines()]
            timestamps = [row[0] for row in data]
            dates = [datetime.datetime.fromtimestamp(ts) for ts in timestamps]
            ...
            plt.legend()
            plt.ylim(bottom=0)
            plt.savefig(path + "/commits_by_author.png")
            plt.close()
