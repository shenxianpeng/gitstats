...
.. note::
    ...
    从 ``2.0.0`` 版本开始，gitstats 使用 `Chart.js <https://www.chartjs.org/>`_ （随报告一起打包）来渲染图表，以实现更现代的可视化效果和更好的性能。
...
.. tip::
    ...
    也可以使用 `uv <https://docs.astral.sh/uv/>`_ 一键下载并运行 gitstats：
...
同时生成 HTML 和 JSON 格式的报告：

.. code-block:: bash

    gitstats . report --format json

这会在 HTML 报告旁边生成一个 ``report.json`` 文件。
...
.. code-block:: bash
    ...
    gitstats [选项] <git路径> <输出路径>

**参数：**

- ``<git路径>`` - Git 仓库的路径（例如 ``.`` 表示当前目录）
- ``<输出路径>`` - 生成报告的目录
